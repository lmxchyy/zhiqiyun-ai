...
func newWithStoreSessionsKnowledgeAndMedia(cfg config.Config, store platformStore, sessions authSessionStore, knowledge *knowledgeModule, mediaRepo mediaRepository, redisClient *redis.Client) *http.Server {
	...
	v1.GET("/ppt/skills", wrapF(api.listPPTSkills))
	v1.POST("/ppt/sessions", wrapF(api.createPPTSession))
	v1.POST("/ppt/sessions/:taskId/messages", wrapF(api.postPPTSessionMessage))
	v1.POST("/ppt/sessions/:taskId/import-outline", wrapF(api.importPPTSessionOutline))
	v1.POST("/ppt/sessions/:taskId/revise-slide", wrapF(api.revisePPTSessionSlide))
	v1.POST("/ppt/sessions/:taskId/confirm-outline", wrapF(api.confirmPPTSessionOutline))
	v1.POST("/ppt/sessions/:taskId/cancel", wrapF(api.cancelPPTSession))
