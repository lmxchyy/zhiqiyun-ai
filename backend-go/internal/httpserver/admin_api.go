...
func billingCustomerRows(data adminPlatformData) []map[string]any {
	...
	for _, user := range data.Users {
		...
		items = append(items, map[string]any{
			...
			"prepaidBalanceCents": points[user.ID].Available * pointUnitAmountCents,
...
func billingSubscriptionRows(data adminPlatformData) []map[string]any {
	...
	for _, user := range data.Users {
		...
		items = append(items, map[string]any{
			...
			"prepaidBalanceCents":  points[user.ID].Available * pointUnitAmountCents,
			"lifetimeUsageCents":   points[user.ID].Frozen * pointUnitAmountCents,
...
func billingWalletRows(data adminPlatformData) []map[string]any {
	...
	for _, account := range data.PointAccounts {
		...
		items = append(items, map[string]any{
			...
			"rateAmount": float64(pointUnitAmountCents) / 100, "priority": 50,
			"balanceCents": account.Available * pointUnitAmountCents, "consumedAmountCents": account.Frozen * pointUnitAmountCents,
