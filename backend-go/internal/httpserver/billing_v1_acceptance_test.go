...
func TestBillingCenterV1Acceptance(t *testing.T) {
	...
	t.Run("old task keeps old price and new task uses published version", func(t *testing.T) {
		...
		if newTask.QuotedPoints != 1200 || newTask.BillingRuleVersionID != published.ID {
			t.Fatalf("new task = quote %.0f version %s, want 1200/%s", newTask.QuotedPoints, newTask.BillingRuleVersionID, published.ID)
		}
	})
}
