...
var errInspirationNotFound = errors.New("inspiration template not found")
...
func newMemoryInspirationRepository() inspirationRepository {
	...
	makeTemplate := func(id, title, description, contentType, categoryID, cover, prompt, negative, model string, params map[string]any, sortOrder int) inspirationTemplate {
		return inspirationTemplate{ID: id, TenantID: "default", Title: title, Description: description, ContentType: contentType, CategoryID: categoryID, CoverURL: cover, ThumbnailURL: cover, ResultURL: cover, Prompt: prompt, NegativePrompt: negative, ModelID: model, Parameters: params, ReferenceAssets: []any{}, Platforms: []string{"miniprogram", "h5", "app", "pc"}, Tags: []string{"AI生成示例"}, Featured: true, Hot: sortOrder >= 60, SortOrder: sortOrder, Status: "PUBLISHED", AuditStatus: "APPROVED", SourceAuthorized: true, Version: 1, CreatedBy: "system", UpdatedBy: "system", CreatedAt: now, UpdatedAt: now}
	}
	templates := []inspirationTemplate{
		makeTemplate("inspiration-product-clean", "极简科技商品主图", "干净背景与商业级产品光影", "image", categories[1].ID, "/static/fallbacks/inspiration-ecommerce.jpg", "高端科技产品商业摄影，产品居中，柔和轮廓光，干净渐变背景，细节清晰，电商主图构图", "文字，水印，低清晰度，畸变，杂乱背景", "gpt-image-2", map[string]any{"ratio": "1:1", "quality": "high", "count": 1}, 100),
		makeTemplate("inspiration-poster-brand", "品牌新品发布海报", "适合新品上市与社交媒体传播", "image", categories[2].ID, "/static/fallbacks/inspiration-poster.jpg", "品牌新品发布视觉海报，现代构图，强烈视觉焦点，留出中文标题排版空间，商业广告质感", "水印，错别字，模糊，廉价素材感", "gpt-image-2", map[string]any{"ratio": "2:3", "quality": "high", "count": 1}, 90),
		makeTemplate("inspiration-portrait-office", "职场形象写真", "自然可信的专业商务形象", "image", categories[3].ID, "/static/fallbacks/default-inspiration.jpg", "专业职场人物肖像，真实自然肤质，柔和棚拍光线，简洁办公背景，可信亲和，高级商业摄影", "过度磨皮，畸形五官，多余手指，水印", "gpt-image-2", map[string]any{"ratio": "2:3", "quality": "high", "count": 1}, 80),
		makeTemplate("inspiration-brand-identity", "未来感品牌视觉", "品牌主视觉与社交传播延展", "image", categories[4].ID, "/static/fallbacks/inspiration-store.jpg", "未来科技品牌主视觉，几何秩序，蓝紫与青色点缀，高级材质，适合企业品牌传播", "水印，拥挤排版，低对比度，模糊", "gpt-image-2", map[string]any{"ratio": "3:2", "quality": "high", "count": 1}, 70),
