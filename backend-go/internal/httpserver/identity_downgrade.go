...
type identityDowngradeAPI struct{ store platformStore }

func newIdentityDowngradeAPI(store platformStore) identityDowngradeAPI {
	return identityDowngradeAPI{store: store}
}

func (a identityDowngradeAPI) preview(w http.ResponseWriter, r *http.Request) {
	store, ok := a.store.(adminIdentityDowngradeStore)
	if !ok {
		writeError(w, http.StatusNotImplemented, errors.New("identity downgrade store is unavailable"))
		return
	}
	...
	result, err := store.PreviewAdminIdentityDowngrade(actorID, actorRole, strings.TrimSpace(r.PathValue("id")), request)
	if err != nil {
...
func (a identityDowngradeAPI) confirm(w http.ResponseWriter, r *http.Request) {
	...
	actorID, actorRole := actorFromRequest(r)
	result, err := store.ConfirmAdminIdentityDowngrade(actorID, actorRole, strings.TrimSpace(r.PathValue("id")), request)
	if err != nil {
		writeIdentityDowngradeError(w, err)
		return
	}
...
func (a identityDowngradeAPI) list(w http.ResponseWriter, r *http.Request) {
	...
	actorID, actorRole := actorFromRequest(r)
	items, err := store.ListAdminIdentityDowngrades(actorID, actorRole, strings.TrimSpace(r.PathValue("id")))
	if err != nil {
		writeIdentityDowngradeError(w, err)
		return
	}
