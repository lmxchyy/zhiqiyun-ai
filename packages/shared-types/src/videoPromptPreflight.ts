...
const COMPLEXITY_SIGNAL_PATTERNS: Array<[string, RegExp]> = [
  ...
  ["multi_shot", /多镜头|多个镜头|镜头切换|multi[-\s]?shot|multiple\s+shots|shot\s+list/iu],
...
export function inspectVideoPromptPreflight(input: VideoPromptPreflightInput): VideoPromptPreflightResult {
  ...
  const requestedDurations = [...prompt.matchAll(DURATION_PATTERN)]
  ...
  if (complexitySignals.length >= 3) {
    warnings.push({
      ...
      message: "提示词同时包含多场景、多镜头、字幕或音频等复杂要求，上游生成失败概率可能更高，建议适当拆分需求。",
      severity: "warning",
    });
  }
...
export function videoGenerationFailureMessage(value: unknown, fallback = "生成失败，请稍后重试") {
  ...
  const billingCopy = billingStatus === "RELEASED" || releasedPoints > 0
    ? "积分已退回/释放。"
    : "积分状态将按现有规则处理。";
