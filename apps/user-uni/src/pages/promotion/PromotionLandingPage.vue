...
onLoad(async options => { const referral = capturePromotionReferral(options as Record<string, unknown>); message.value = referral ? "推广信息已安全保存" : "正在打开小程序"; if (!getAuthToken()) { setTimeout(() => uni.reLaunch({ url: "/pages/WechatLoginPage" }), 300); return; } await syncPendingPromotionReferral(); uni.switchTab({ url: "/pages/user/UserHomePage", fail: () => uni.reLaunch({ url: "/pages/user/UserHomePage" }) }); });
</script>
