...
async function prepare(invalidate: boolean) { loading.value = true; error.value = ""; try { await userStore.loadProfile(); overview.value = await promotionAPI.overview(userStore.userId, userStore.tenantId, userStore.currentRole, invalidate); let codePayload: PromotionCode; let copy = await promotionAPI.shareCopy(templateId.value); if (userStore.currentRole === "AGENT") { const agentPoster = await promotionAPI.agentPoster(); codePayload = { imageDataUrl: agentPoster.qrCodeDataUrl, scene: agentPoster.inviteCode, page: agentPoster.inviteLink, isPlaceholder: false, cacheKey: `agent-h5-${agentPoster.inviteCode}`, expiresAt: "" }; copy = { ...copy, text: `${copy.text}\n安卓 APP 专属注册下载：${agentPoster.inviteLink}`, path: agentPoster.inviteLink }; } else { codePayload = await promotionAPI.code({ userId: userStore.userId, tenantId: userStore.tenantId, currentRole: userStore.currentRole, templateId: templateId.value, activityId: overview.value.activity?.id, invalidate }); } await promotionAPI.renderConfig(templateId.value, overview.value.activity?.id); code.value = codePayload; shareCopy.value = copy; qrPath.value = await imageDataUrlToLocalPath(codePayload.imageDataUrl, codePayload.cacheKey); loading.value = false; await nextTick(); if (!canvasRef.value) throw new Error("海报画布初始化失败"); posterPath.value = await canvasRef.value.exportPoster(); trackPromotion("promotion_poster_generate", { templateId: templateId.value }); } catch (reason) { error.value = reason instanceof Error ? reason.message : "请稍后重试"; } finally { loading.value = false; } }
