<template>
  <view class="promotion-page">
    <PromotionPageHeader title="我的推广码" subtitle="分享知启云AI，与好友一起高效创作" />
...
onLoad(options => { const id = promotionTemplateById(String(options?.templateId || "")).id; selectedTemplateId.value = id; });
onShow(() => void load(false));
...
async function load(force: boolean) {
  ...
  } catch (reason) { error.value = reason instanceof Error ? reason.message : "请稍后重试"; }
...
async function loadCode(invalidate = false) {
  ...
  } catch (reason) { uni.showToast({ title: reason instanceof Error ? reason.message : "小程序码生成失败", icon: "none" }); }
  finally { codeLoading.value = false; }
}

async function loadShareCopy() { try { shareCopy.value = await promotionAPI.shareCopy(selectedTemplateId.value); } catch { shareCopy.value = null; } }
...
function openStats() { trackPromotion("promotion_stats_view"); uni.navigateTo({ url: "/pages/promotion/PromotionStatsPage" }); }
function trackShare() { trackPromotion("promotion_share", { templateId: selectedTemplateId.value, channel: "wechat_friend" }); }

onShareAppMessage(() => ({ title: shareCopy.value?.title || "知启云AI，让创意更高效", path: shareCopy.value?.path || `/pages/promotion/PromotionLandingPage?invite=${overview.value?.profile.inviteCode || ""}&templateId=${selectedTemplateId.value}` }));
onShareTimeline(() => ({ title: shareCopy.value?.title || "知启云AI，让创意更高效", query: `invite=${overview.value?.profile.inviteCode || ""}&templateId=${selectedTemplateId.value}&source=moments` }));
</script>
