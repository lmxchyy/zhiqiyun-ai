<template>
  <view class="mpb-page">
    ...
    <view class="mpb-stack">
      <view class="mpb-hero"><view class="mpb-hero-top"><view><text class="mpb-hero-label">推广转化率</text><text class="mpb-hero-value">{{ conversionRate }}%</text></view><text class="mpb-hero-badge">邀请码 {{ inviteCode || '-' }}</text></view><text class="mpb-hero-copy">专属推广链接的访问、注册和成交数据均来自服务端渠道中心。</text><view class="mpb-hero-metrics"><view class="mpb-hero-metric"><text class="mpb-hero-metric-value">{{ stat('visits') }}</text><text class="mpb-hero-metric-label">访问</text></view><view class="mpb-hero-metric"><text class="mpb-hero-metric-value">{{ stat('registrations') }}</text><text class="mpb-hero-metric-label">注册</text></view><view class="mpb-hero-metric"><text class="mpb-hero-metric-value">{{ stat('orders') }}</text><text class="mpb-hero-metric-label">成交</text></view></view></view>
...
import { asRecord, backOrHome, formatDate, listOf, rowNumber, rowString, type AnyRecord } from "../../utils/miniProgramBusiness";
import loginLogo from "../../assets/zhiqiyun-logo-transparent.png";
const customers = ref<AnyRecord[]>([]); const inviteCode = ref(""); const inviteLink = ref(""); const summary = ref<AnyRecord>({});
const conversionRate = computed(() => stat("visits") > 0 ? Math.round(stat("orders") / stat("visits") * 100) : 0);
function stat(key: string) { return rowNumber(summary.value, key); }
async function load() { try { const center = await businessSdk.roleWorkbench.channelCenter(); customers.value = listOf(center.customers); inviteCode.value = rowString(center.agent, "inviteCode"); inviteLink.value = rowString(center.agent, "inviteLink") || rowString(asRecord(center).promotion, "inviteLink", "landingURL"); summary.value = asRecord(center.summary); } catch (error) { uni.showToast({ title: error instanceof Error ? error.message : "邀请记录加载失败", icon: "none" }); } }
...
function open(item: AnyRecord) { uni.navigateTo({ url: `/pages/agent/AgentCustomerDetailPage?id=${encodeURIComponent(rowString(item, 'id'))}` }); }
onShow(load);
</script>
