  ...
  <view class="mpb-page">
    ...
    <view class="mpb-stack">
      ...
      <template v-else-if="customer">
        ...
        <view class="mpb-card"><text class="mpb-card-title">最近订单</text><view v-if="orders.length" class="mpb-list"><view v-for="item in orders.slice(0, 5)" :key="orderId(item)" class="mpb-row"><view class="mpb-row-main"><text class="mpb-row-title">{{ orderTitle(item) }}</text><text class="mpb-row-meta">{{ formatDate(rowString(item, 'createdAt')) }}</text></view><text class="mpb-amount">{{ formatCurrency(orderAmount(item)) }}</text></view></view><text v-else class="mpb-card-copy">暂无客户订单。</text></view>
