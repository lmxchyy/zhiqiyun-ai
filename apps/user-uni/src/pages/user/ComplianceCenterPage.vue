<template>
  <view class="compliance-page">
    <view class="compliance-header">
      ...
      <view><text class="title">协议与安全</text><text class="subtitle">知启云AI小程序合规信息</text></view>
    </view>
    ...
    <view v-else class="document-list">
      <button v-for="item in documents" :key="item.code" class="document" @click="openDocument(item)">
        <view><text class="document-title">{{ item.title }}</text><text class="document-version">版本 {{ item.version }}</text></view><text>›</text>
      </button>
      <button class="document" @click="openExternal(complaintUrl, '投诉举报')"><text class="document-title">投诉举报</text><text>›</text></button>
      <button class="document" @click="openExternal(infringementUrl, '侵权投诉')"><text class="document-title">侵权投诉</text><text>›</text></button>
      <view v-if="acceptanceItems.length" class="acceptance-card">
        <text class="acceptance-title">首次生成前协议确认</text>
        <text class="acceptance-summary">{{ acceptanceReady ? '当前必要协议已确认' : '请阅读并确认当前发布版本；协议更新后需要重新确认。' }}</text>
        <view v-for="item in acceptanceItems" :key="`accept-${item.code}`" class="acceptance-row">
          <text>{{ item.title }}（{{ item.version }}）</text><text :class="item.accepted ? 'accepted' : 'pending'">{{ item.accepted ? '已确认' : '待确认' }}</text>
        </view>
        <button v-if="!acceptanceReady" class="accept-button" :disabled="accepting" @click="acceptCurrent">{{ accepting ? '正在提交...' : '我已阅读并同意当前必要协议' }}</button>
      </view>
    </view>
    <view v-if="active" class="sheet" @click="active = null">
      <view class="sheet-card" @click.stop>
        <text class="sheet-title">{{ active.title }}</text><text class="sheet-version">版本 {{ active.version }}</text>
        <scroll-view scroll-y class="sheet-content"><text>{{ active.content }}</text></scroll-view>
        <button class="close" @click="active = null">我知道了</button>
      </view>
    </view>
  </view>
</template>
...
onMounted(async () => {
  try {
    const payload = await apiRequestTask<{ items: LegalDocument[]; complaintUrl: string; infringementUrl: string }>("/api/v1/public/legal-documents", { auth: false }).promise;
    documents.value = payload.items || [];
    complaintUrl.value = payload.complaintUrl || "";
    infringementUrl.value = payload.infringementUrl || "";
    try {
    ...
    } catch { /* 游客仍可浏览公开协议，登录后再显示确认状态。 */ }
  } catch (reason) {
    error.value = reason instanceof Error ? reason.message : "合规信息加载失败";
  } finally { loading.value = false; }
});

function back() { uni.navigateBack({ fail: () => uni.switchTab({ url: "/pages/user/UserMinePage" }) }); }
function openDocument(item: LegalDocument) { active.value = item; }
...
function openExternal(value: string, title: string) {
  if (!value || value === "待配置") { uni.showToast({ title: `${title}入口待运营配置`, icon: "none" }); return; }
  // #ifdef H5
