<template>
  <view :class="['asset-card', { selected, selecting }]" role="button" :aria-label="asset.name" :data-asset-id="asset.id" @click="handleOpen">
    <view class="asset-card-cover">
      <AssetCover :asset="asset" @thumbnail-load="$emit('thumbnail-load', $event)" />
...
const emit = defineEmits<{ open:[asset:AssetItem]; action:[asset:AssetItem]; favorite:[asset:AssetItem]; select:[asset:AssetItem]; "thumbnail-load":[assetID:string] }>();
