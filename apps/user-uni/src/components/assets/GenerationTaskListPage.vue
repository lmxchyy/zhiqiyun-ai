<template>
  <view class="task-page" :style="miniProgramNavigationStyle">
    ...
    <view v-else-if="store.tasks.length" class="task-list"><GenerationTaskItem v-for="task in store.tasks" :key="task.id" :task="task" @open="openTask" @cancel="cancel" @retry="retry" @delete="remove" @result="result"/></view>
...
onMounted(() => { void store.fetchTasks(true); store.startTaskPolling(0); });
...
function cancel(task:GenerationTask){uni.showModal({title:"取消任务",content:"确定停止当前生成任务吗？",confirmColor:"#ff771b",success:res=>{if(res.confirm){uni.showLoading({title:"正在取消",mask:true});void store.cancelTask(task.id).then(()=>uni.showToast({title:"已提交取消",icon:"success"})).catch(error=>uni.showToast({title:error instanceof Error?error.message:"取消失败，请稍后重试",icon:"none"})).finally(()=>uni.hideLoading());}}});}
...
async function executeDelete(task:GenerationTask){uni.showLoading({title:"正在删除",mask:true});try{await store.deleteTask(task.id);uni.hideLoading();uni.showToast({title:"已删除",icon:"success"});}catch(error){uni.hideLoading();uni.showToast({title:error instanceof Error?error.message:"删除失败",icon:"none"});}}
function retry(task:GenerationTask){confirmRetryFailedTask(task,executeRetry);}
function remove(task:GenerationTask){confirmDeleteFailedTask(task,executeDelete);}
...
const disposeNativeBridge=registerAssetNativeBridge({
  ...
  deleteTask:id=>{const task=taskById(id);if(task)remove(task);},
  openTaskResult:id=>{const task=taskById(id);if(task)result(task);},
});
onBeforeUnmount(disposeNativeBridge);
onBeforeUnmount(() => { store.stopTaskPolling(); });
</script>
<style scoped>.task-page{min-height:100vh;box-sizing:border-box;padding:0 16px calc(24px + env(safe-area-inset-bottom));color:#1a1f2e;background:#f7f8fc}.safe-top{height:var(--header-padding-top,20px)}.task-header{display:flex;min-height:var(--navigation-bar-height,44px);padding-right:var(--capsule-right-space,0px);box-sizing:border-box;align-items:center;gap:11px}.task-header button{width:36px;height:36px;padding:0;border:0;border-radius:12px;color:#5a4db2;background:#fff;font-size:28px}.task-header>view{display:flex;min-width:0;flex:1;flex-direction:column}.task-header>view text:first-child{font-size:20px;font-weight:720}.task-header>view text:last-child{margin-top:2px;color:#8a90a1;font-size:9px}.task-header>text{padding:5px 9px;border-radius:12px;color:#5a4db2;background:#fff;font-size:10px}.task-list{display:flex;margin-top:12px;flex-direction:column;gap:9px}.task-loading{display:flex;min-height:280px;align-items:center;justify-content:center;color:#8a90a1;font-size:11px}.load-state{display:flex;min-height:72px;align-items:center;justify-content:center;color:#8b91a5;font-size:10px}.load-state button{height:34px;padding:0 17px;border:0;border-radius:17px;color:#5a4db2;background:#fff;font-size:10px}.bottom-safe{height:16px}</style>
