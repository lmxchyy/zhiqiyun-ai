<template>
  <view :class="['asset-cover-shell', `type-${asset.type}`]">
    ...
    <text :class="['cover-type-tag', { 'is-queued': asset.status === 'queued' }]">{{ coverTag }}</text>
...
const coverTag = computed(() => ({ queued:GENERATION_QUEUED_LABEL,generating:"生成中",failed:"生成失败" } as Record<string,string>)[props.asset.status] || label.value);
...
.cover-type-tag.is-queued{max-width:calc(100% - 16px)}
</style>
