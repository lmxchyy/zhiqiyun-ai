<template>
  <view class="asset-center-page">
    ...
    <view v-if="store.showingCachedWorks && store.offline && store.assets.length" class="cached-content-note"><text>当前展示缓存内容</text></view>
    ...
    <AssetGrid v-else-if="store.assets.length" class="asset-result-block" :assets="store.assets.slice(0,4)" @open="openAsset" @action="activeAsset=$event" @favorite="toggleFavorite" @thumbnail-load="handleThumbnailLoad" />
    ...
    <view v-if="store.refreshing && !store.assets.length" class="refresh-indicator"><text>正在刷新作品...</text></view>
...
const componentCreatedAt=Date.now();let firstCardsRendered=false;let firstThumbnailLoaded=false;
function refresh(){return store.isDefaultRecentView?store.refreshRecentWorks({source:"AssetCenterPage.retry",force:true,silent:Boolean(store.assets.length)}):store.refreshAssets(4);}function changeType(value:AssetType){void store.setType(value,4);}function changeStatus(value:AssetStatus){void store.setStatus(value,4);}function changeSort(value:AssetSort){void store.setSort(value,4);}function applyFilters(value:AssetFilter){void store.setFilters(value,4);}
watch(()=>store.assets.length,async count=>{if(!count||firstCardsRendered)return;await nextTick();firstCardsRendered=true;recordWorksPerformance("works_cards_rendered",worksTabClickStartedAt()||componentCreatedAt,Date.now(),{serialWait:false,source:"AssetCenterPage",cacheHit:store.showingCachedWorks,itemCount:count});},{immediate:true});
function handleThumbnailLoad(assetID:string){if(firstThumbnailLoaded)return;firstThumbnailLoaded=true;recordWorksPerformance("first_thumbnail_loaded",worksTabClickStartedAt()||componentCreatedAt,Date.now(),{serialWait:false,source:"AssetCenterPage",itemCount:1,note:assetID});}
