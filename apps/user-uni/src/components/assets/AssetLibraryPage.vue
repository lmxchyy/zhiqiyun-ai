<template>
  <view class="library-page" :style="miniProgramNavigationStyle">
    <view class="safe-top" />
...
function openAsset(asset:AssetItem){openDetail(asset);}function openDetail(asset:AssetItem){uni.navigateTo({url:`${miniProgramFeaturePages.userAssetDetail}?id=${encodeURIComponent(asset.id)}`});}function continueEditing(asset:AssetItem){const mode=asset.type==="video"?"video":asset.type==="ppt"?"ppt":asset.type==="agent"||asset.type==="knowledge"?"agent":asset.type==="infographic"?"infographic":"image";uni.navigateTo({url:`${miniProgramAssetCreationPages[mode]}?assetId=${encodeURIComponent(asset.id)}&intent=edit`});}async function toggleFavorite(asset:AssetItem){try{await store.toggleFavorite(asset.id);}catch{/* rollback handled in store */}}
