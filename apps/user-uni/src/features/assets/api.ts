...
import { normalizeTaskLifecycleStatus } from "./taskLifecycle";
...
function normalizeTask(value: unknown): GenerationTask {
  ...
  const normalizedStatus = normalizeTaskLifecycleStatus(raw.status || "PENDING");
  const status: GenerationTask["status"] = normalizedStatus === "running" ? "generating" : normalizedStatus;
  return {
