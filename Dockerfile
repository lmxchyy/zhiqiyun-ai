...
RUN if [ -n "$ALPINE_MIRROR" ]; then \
  ...
  && apk add --no-cache ca-certificates curl bash icu-libs tzdata python3 py3-pip \
  && apk add --no-cache ffmpeg=6.1.1-r8 font-noto-cjk \
...
COPY --from=api-build /out/video-backfill /app/video-backfill
