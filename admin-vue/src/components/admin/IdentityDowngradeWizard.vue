<template>
  <el-dialog v-model="visible" width="min(900px, 94vw)" :close-on-click-modal="false" destroy-on-close @closed="reset">
    ...
    <el-form ref="formRef" :model="form" :rules="rules" label-position="top">
      <section v-show="step === 0" class="section">
        ...
        <el-form-item label="下级处理方式" prop="childStrategy">
          <el-radio-group v-model="form.childStrategy" class="strategy-grid">
            ...
            <el-radio border value="PRESERVE_HISTORY"><strong>停止新增分润</strong><small>结束有效关系，完整保留历史记录</small></el-radio>
          </el-radio-group>
        </el-form-item>
      ...
      <section v-show="step === 1" class="section" v-loading="previewing">
        <template v-if="preview">
          ...
          <div class="metric-grid"><article><span>下级会员</span><strong>{{ preview.downlineMembers }}</strong></article><article><span>下级代理商</span><strong>{{ preview.downlineAgents }}</strong></article><article><span>预计迁移关系</span><strong>{{ preview.migrationCount }}</strong></article><article><span>生效时间</span><strong class="date-value">{{ dateTime(preview.effectiveAt) }}</strong></article></div>
          ...
          <el-descriptions :column="2" border><el-descriptions-item label="下级策略">{{ strategyLabel(preview.childStrategy) }}</el-descriptions-item><el-descriptions-item label="等待自动降级">{{ preview.waitForSettlement ? '是' : '否' }}</el-descriptions-item><el-descriptions-item label="操作原因" :span="2">{{ form.reason }}</el-descriptions-item></el-descriptions>
        </template>
    ...
    <template #footer><el-button @click="visible=false">取消</el-button><el-button v-if="step>0" :disabled="submitting" @click="step--">上一步</el-button><el-button v-if="step<2" type="primary" :loading="previewing" :disabled="step===1 && preview?.status==='BLOCKED'" @click="next">下一步</el-button><el-button v-else type="danger" :loading="submitting" :disabled="!confirmed" @click="confirmDowngrade">确认降级</el-button></template>
  </el-dialog>
</template>
...
const emit=defineEmits<{success:[]}>();const visible=defineModel<boolean>({required:true});const formRef=ref<FormInstance>();const step=ref(0);const preview=ref<IdentityDowngradePreview|null>(null);const previewing=ref(false);const submitting=ref(false);const confirmed=ref(false);const errorMessage=ref("");
const form=reactive({targetIdentity:"" as ""|"AGENT",childStrategy:"PRESERVE_HISTORY" as IdentityDowngradeStrategy,targetAgentId:"",targetOperationCenterId:"",scheduled:false,effectiveAt:"",waitForSettlement:false,reason:"",remark:""});
...
function reset(){step.value=0;preview.value=null;previewing.value=false;submitting.value=false;confirmed.value=false;errorMessage.value="";Object.assign(form,{targetIdentity:props.currentIdentity==="OPERATION_CENTER"?"AGENT":"",childStrategy:"PRESERVE_HISTORY",targetAgentId:"",targetOperationCenterId:"",scheduled:false,effectiveAt:"",waitForSettlement:false,reason:"",remark:""})}
