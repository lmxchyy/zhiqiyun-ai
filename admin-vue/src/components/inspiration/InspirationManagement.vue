  ...
  <section class="inspiration-admin">
    ...
    <el-drawer v-model="editorOpen" size="720px" :title="editingId ? '编辑创作灵感' : '新增创作灵感'" destroy-on-close>
      <el-form label-position="top" class="editor-form">
        ...
        <div class="form-grid"><el-form-item label="分类" required><el-select v-model="form.categoryId"><el-option v-for="item in categories" :key="item.id" :label="item.name" :value="item.id"/></el-select></el-form-item><el-form-item label="绑定模型"><el-input v-model="form.modelId" placeholder="模型中心中的 modelId"/></el-form-item></div>
        ...
        <el-form-item label="完整提示词" required><el-input v-model="form.prompt" type="textarea" :rows="5" maxlength="8000" show-word-limit/></el-form-item>
        <el-form-item label="负面提示词"><el-input v-model="form.negativePrompt" type="textarea" :rows="3" maxlength="3000" show-word-limit/></el-form-item>
        <el-form-item label="生成参数 JSON"><el-input v-model="parametersText" type="textarea" :rows="5" placeholder='{"ratio":"1:1","quality":"high"}'/></el-form-item>
        <el-form-item label="参考素材 JSON"><el-input v-model="referenceAssetsText" type="textarea" :rows="3" placeholder='[]'/></el-form-item>
...
import { inspirationAdminAPI, type InspirationCategory, type InspirationTemplate } from "../../api/inspirations";
...
const emptyForm=()=>({title:"",description:"",contentType:"image" as const,categoryId:"",coverUrl:"",thumbnailUrl:"",resultUrl:"",prompt:"",negativePrompt:"",modelId:"",parameters:{} as Record<string,unknown>,referenceAssets:[] as unknown[],platforms:["miniprogram"],tags:[] as string[],applicableTenantIds:[] as string[],featured:false,hot:false,pinned:false,sort:0,status:"DRAFT",auditStatus:"PENDING",sourceAssetId:"",sourceAuthorized:false,startTime:"",endTime:""});
const form=reactive(emptyForm());const parametersText=ref("{}"),referenceAssetsText=ref("[]");
...
function openCreate(){editingId.value="";Object.assign(form,emptyForm());form.categoryId=categories.value[0]?.id||"";parametersText.value="{}";referenceAssetsText.value="[]";editorOpen.value=true}
function openEdit(item:InspirationTemplate){editingId.value=item.id;Object.assign(form,item);parametersText.value=JSON.stringify(item.parameters||{},null,2);referenceAssetsText.value=JSON.stringify(item.referenceAssets||[],null,2);editorOpen.value=true}
...
async function save(){if(!form.title.trim()||!form.categoryId||!form.coverUrl.trim()||!form.prompt.trim()){ElMessage.warning("请完整填写标题、分类、封面和提示词");return}try{form.parameters=JSON.parse(parametersText.value||"{}");form.referenceAssets=JSON.parse(referenceAssetsText.value||"[]")}catch{ElMessage.error("参数或参考素材不是有效 JSON");return}saving.value=true;try{editingId.value?await inspirationAdminAPI.update(editingId.value,form):await inspirationAdminAPI.create(form);ElMessage.success("模板已保存");editorOpen.value=false;await load()}catch(e){ElMessage.error((e as Error).message)}finally{saving.value=false}}
