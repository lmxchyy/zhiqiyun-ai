<template>
  <div class="web-login-form">
    ...
    <label class="web-login-agreement" :class="{ 'is-error': Boolean(formError) && !agreementAccepted }">
    ...
    <p v-if="formError" class="web-login-inline-error" role="alert">{{ formError }}</p>
    ...
    <section v-if="mode === 'wechat'" class="wechat-login-panel" aria-live="polite">
      ...
      <div v-else-if="qrUrl && qrStatus !== 'EXPIRED' && qrStatus !== 'UNAVAILABLE'" class="wechat-qr-frame-wrap">
        <iframe class="wechat-qr-frame" :src="qrUrl" title="微信扫码登录二维码" referrerpolicy="origin" />
...
function requireAgreement() {
  ...
  formError.value = "请先勾选「我已阅读并同意用户协议和隐私政策」";
...
async function submitPasswordLogin() {
  ...
  if (!account || !passwordForm.password.trim()) {
    formError.value = "请输入手机号或邮箱和密码";
    ElMessage.warning(formError.value);
    return;
  }
  formError.value = "";
...
async function bindWechatMobile() {
  if (submitting.value || !requireAgreement()) return;
  if (!validMobile(bindMobileForm.mobile) || !/^\d{4,6}$/.test(bindMobileForm.code)) {
    formError.value = "请输入正确的手机号和验证码";
    ElMessage.warning(formError.value);
    return;
  }
  formError.value = "";
  ...
  try { emit("authenticated", await adminRequest<AuthResponse>({ method: "POST", url: "/auth/wechat/bind-mobile", authMode: "none", retryOnUnauthorized: false, data: { qrCodeId: qrCodeId.value, mobile: bindMobileForm.mobile.trim(), smsCode: bindMobileForm.code.trim() } }), remember.value); }
    ...
    formError.value = error instanceof Error ? error.message : "手机号绑定失败";
    ElMessage.error(formError.value);
  }
  finally { submitting.value = false; }
}
...
watch(mode, (value) => { formError.value = ""; if (value === "wechat" && agreementAccepted.value && !qrCodeId.value) void loadQRCode(); else if (value !== "wechat") stopQRPolling(); });
watch(agreementAccepted, (accepted) => {
  if (accepted) formError.value = "";
  if (accepted && mode.value === "wechat" && !qrCodeId.value) void loadQRCode();
  if (!accepted) stopQRPolling();
});
