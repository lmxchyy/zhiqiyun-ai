...
# Keep plan model on a real chat model.
...
docker exec zhiqiyun-ai-prod-postgres-1 psql -U zhiqiyun_prod -d zhiqiyun -v ON_ERROR_STOP=1 -c \
  "UPDATE video_projects SET status='MATERIAL_READY', active_analysis_task_id=NULL, active_plan_task_id=NULL, error_stage=NULL, error_code=NULL, error_message=NULL, updated_at=now() WHERE id='vp_664248192f84dc96631df8cd';"

docker exec zhiqiyun-ai-prod-postgres-1 psql -U zhiqiyun_prod -d zhiqiyun -P pager=off -c \
  "SELECT id,status,error_stage,error_code FROM video_projects WHERE id='vp_664248192f84dc96631df8cd';"

docker exec zhiqiyun-ai-prod-xianzhi-ai-1 printenv SMARTVIDEO_PLAN_MODEL || true
docker exec zhiqiyun-ai-prod-smartvideo-worker-1 printenv SMARTVIDEO_PLAN_MODEL || true
